/**
 * 数据库初始化 + 种子数据
 * 本地：node src/db/seed.js
 * 线上：DATABASE_URL=xxx node src/db/seed.js
 */
...
async function main() {
  ...
  await db.exec(`
    CREATE TABLE IF NOT EXISTS shops (
      ...
      name TEXT NOT NULL ${isPg ? 'UNIQUE' : 'UNIQUE'}
    )
  `)
  ...
  if (existingCount > 0) {
    console.log(`✅ 数据库已存在 ${existingCount} 条订单，跳过初始化`)
    process.exit(0)
  }
  ...
  for (let daysAgo = 0; daysAgo <= 59; daysAgo++) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const dateStr = fmtD(day).replace(/-/g, '')
      const orderNo = `MT${dateStr}${String(seq++).padStart(4, '0')}`
  ...
  // 统计
  const totalRow = await db.queryOne('SELECT COUNT(*) as cnt FROM orders')
  const stats    = await db.query('SELECT order_status, COUNT(*) as cnt FROM orders GROUP BY order_status ORDER BY cnt DESC')
  const minRow   = await db.queryOne('SELECT MIN(create_time) as d FROM orders')
  const maxRow   = await db.queryOne('SELECT MAX(create_time) as d FROM orders')

  console.log(`
✅ 数据库初始化完成！（${isPg ? 'PostgreSQL' : 'SQLite'}）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📦 订单总数：${parseInt(totalRow.cnt)} 条
📅 时间范围：${minRow.d?.slice(0,10)} ~ ${maxRow.d?.slice(0,10)}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
状态分布：
${stats.map(s => `  ${s.order_status.padEnd(8)}：${s.cnt} 条`).join('\n')}
  `)
}
